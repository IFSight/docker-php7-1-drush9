...
mkdir -p /usr/share/drush/commands /phphome drush9                                             && \
chown php.php /phphome drush9                                                                  && \
su - php -c "echo '{\"minimum-stability\": \"RC\"}' > /usr/local/drush9/composer.json"         && \
su - php -c "cd /usr/local/drush9 && composer require drush/drush:9.6.*"                       && \
ln -s /usr/local/drush9/vendor/drush/drush/drush /usr/local/bin/drush                          && \
